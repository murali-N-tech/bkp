...
// Note: domains will be loaded dynamically via `useDomains` inside the component

const DomainCards = () => {
  const containerRef = useRef(null);
  const [showAll, setShowAll] = useState(false);
  const { domains: fetchedDomains = [], loading } = useDomains();
  const navigate = useNavigate();

  // Transform domains data to include gamification stats (uses fetchedDomains)
  const transformedDomains = (fetchedDomains || []).map((domain, index) => {
    const mockStats = [
    ...
    // Read user enrollments/progress from localStorage.userData (if any)
    let userEnrollments = null;
    try {
      const raw = localStorage.getItem('userData');
      if (raw) {
        const parsed = JSON.parse(raw);
        // possible shapes: parsed.enrollments || parsed.courses || parsed.enrolled
        userEnrollments = parsed.enrollments || parsed.courses || parsed.enrolled || parsed.enrolledCourses || null;
      }
    } catch (e) {
      userEnrollments = null;
    }

    // Build a quick lookup by domain id or title
    let enrollmentForDomain = null;
    if (userEnrollments && Array.isArray(userEnrollments)) {
      enrollmentForDomain = userEnrollments.find(e => {
        // allow matching by id, domainId, title/name
        if (!e) return false;
        if (e.domainId && (e.domainId === domain.id || e.domainId === domain.domainId)) return true;
        if (e.id && (e.id === domain.id || e.id === domain.domainId)) return true;
        const eTitle = e.title || e.name || e.domain;
        if (eTitle && domain.title && eTitle.toLowerCase() === domain.title.toLowerCase()) return true;
        return false;
      }) || null;
    }

    // Determine progress: prefer enrollment.progress or completed/total, else fallback to mock stats
    let computedProgress = stats.progress;
    if (enrollmentForDomain) {
      if (typeof enrollmentForDomain.progress === 'number') {
        computedProgress = Math.max(0, Math.min(100, Math.round(enrollmentForDomain.progress)));
      } else if (typeof enrollmentForDomain.completed === 'number' && typeof enrollmentForDomain.total === 'number' && enrollmentForDomain.total > 0) {
        computedProgress = Math.max(0, Math.min(100, Math.round((enrollmentForDomain.completed / enrollmentForDomain.total) * 100)));
      }
    }
  ...
  const domains = transformedDomains;
  ...
  const handleStartAssessment = (domainId) => {
    if (!domainId) return;
    // Navigate to guest assessment for this domain
  ...
  return (
    <section ref={containerRef} className="py-24 relative bg-background">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Auto-scrolling Domain Carousel - MOVED TO TOP - LARGER WITH ICONS */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        {/* Domain Cards Grid - 4 cards per row, 2 rows initially */}
        <motion.div
          initial={{ opacity: 0 }}
        ...
        >
          {/* Animated background */}
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-brand-600/5 to-transparent"
          ...
          <div className="relative z-10">
            <h3 className="text-4xl font-bold text-foreground mb-4">
              Ready to <span className="text-brand-600">Assess Yourself</span>?
            </h3>
            <p className="text-lg text-muted-foreground mb-8 max-w-2xl mx-auto">
              Click on any domain above to start your assessment journey. Our AI-powered system will adapt to your learning pace and provide personalized feedback to help you improve.
            </p>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                size="lg"
                onClick={() => handleStartAssessment('web-dev')}
